...
def main():
    parser = argparse.ArgumentParser(description="Doc bien so tu anh")
    parser.add_argument("--image", required=True, help="Duong dan anh bien so")
    parser.add_argument("--show", action="store_true", help="Hien thi anh debug")
    args = parser.parse_args()

    plate, raw, score, source, debug_image = detect_plate_text(args.image)
    if not plate:
        print("Khong phat hien duoc bien so")
        return
    ...
    if args.show and debug_image is not None:
